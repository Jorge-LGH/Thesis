...
# Ribosomal genes to object metadata. Will not be used as QC threshold, but is still relevant
...
VlnPlot(rna, features = c("nFeature_RNA", "nCount_RNA", "percent.mt", "percent.Ribosomal"), ncol = 4)

# Scatter plot read count per cell by feature count per cell
# 0.88 correlation between both axes 
plt1 <- FeatureScatter(rna, "nCount_RNA", "nFeature_RNA") + theme(legend.position="none") + xlab("Reads per cell") +
...
plt2 <- FeatureScatter(rna, "nCount_RNA", "percent.mt") + theme(legend.position="none") + xlab("Reads per cell") +
...
plt3 <- qc.metrics %>% ggplot(aes(percent.mt)) + geom_histogram(binwidth = 0.5, fill="yellow", colour="black") +
  xlab("Mitochondrial genes percent") + ylab("Cells")+
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, 
                                                                     linetype = "solid", colour = "black"))

plt4 <- qc.metrics %>% ggplot(aes(percent.Ribosomal)) + geom_histogram(binwidth = 0.5, fill="yellow", colour="black") +
...
# Plot showing data pre-normalization
as_tibble(rna@assays$RNA@data[,1:50]) %>% 
  ...
  stat_summary(geom="crossbar", fun.data=mean_sdl) + ggtitle("Pre-Normalization Gene Expression Distribution") +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, 
                                                                     linetype = "solid", colour = "black"))
...
# Scatter plot read count per cell by feature count per cell
plt5 <- FeatureScatter(rna, "nCount_RNA", "nFeature_RNA") + theme(legend.position="none") + xlab("Reads per cell") +
  ylab("Features per cell")
plt6 <- FeatureScatter(rna, "nCount_RNA", "percent.mt") + theme(legend.position="none") + xlab("Reads per cell") +
  ylab("% of mitochondrial genes")
plt5 + plt6
...
plt7 <- qc.metrics %>% ggplot(aes(percent.mt)) + geom_histogram(binwidth = 0.5, fill="yellow", colour="black") +
  xlab("Mitochondrial genes percent") + ylab("Cells")+
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, 
                                                                     linetype = "solid", colour = "black"))

plt8 <- qc.metrics %>% ggplot(aes(percent.Ribosomal)) + geom_histogram(binwidth = 0.5, fill="yellow", colour="black") +
  xlab("Ribosomal genes percent") + ylab("Cells") +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, 
                                                                     linetype = "solid", colour = "black"))
...
as_tibble(rna@assays$RNA@data[,1:50]) %>%
  pivot_longer(cols=everything(), names_to="cell", values_to="expression") %>%
  ggplot(aes(x=cell, y=expression)) + xlab("Cell ID") + ylab("Gene expression") +
  stat_summary(geom="crossbar", fun.data=mean_sdl) + ggtitle("Normalized Gene Expression Distribution") +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, 
                                                                     linetype = "solid", colour = "black"))
...
LabelPoints(var_genes_plt, points = top10, repel = TRUE, xnudge = 0, ynudge = 0)
